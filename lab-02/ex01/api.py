...
def encrypt():
    ...
    plain_text = data['plain_text']
    key = int(data['key'])
    encrypted_text = railfence_cipher.rail_fence_ecrypt(plain_text, key)
    return jsonify({'encrypted_text': encrypted_text})
...
def decrypt():
    ...
    return jsonify({'encrypted_text': decrypt_text})
#PLAYFAIR
...
#TranspositionCipher
@app.route('/api/transpositioncipher/encrypt', methods=['POST'])
def transposition_encrypt():
    data = request.get_json()
    plain_text = data.get('plain_text')
    key = int(data.get('key'))
    encrypt_text = transposition_cipher.encrypt(plain_text, key)
    return jsonify({'encrypted_text': encrypt_text})
@app.route('/api/transpositioncipher/decrypt', methods=['POST'])
